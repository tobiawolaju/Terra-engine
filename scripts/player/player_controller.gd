...
func _physics_process(delta: float) -> void:
	...
	if _pick_action_available and Input.is_action_just_pressed("pick"):
		if _held_pickable != null:
			...
			if nearest_pickable != null:
				_held_pickable = nearest_pickable
	...
	if _floor_state_initialized:
		if not was_on_floor and on_floor_now and not is_in_water:
...
func _setup_audio_streams() -> void:
	_assign_audio_stream(_audio_bg, BG_STREAM)
	_assign_audio_stream(_audio_run, RUN_STREAM)
	_assign_audio_stream(_audio_jump, JUMP_STREAM)
	_assign_audio_stream(_audio_death, DEATH_STREAM)
	_assign_audio_stream(_audio_splash, SPLASH_STREAM)
	_assign_audio_stream(_audio_swimming, SWIMMING_STREAM)


func _assign_audio_stream(player: AudioStreamPlayer3D, stream: AudioStream) -> void:
	if player == null:
		return
...
func _update_audio_state(move_direction: Vector3, is_in_water: bool) -> void:
	if _is_dead:
		if _audio_run != null and _audio_run.playing:
			_audio_run.stop()
		if _audio_swimming != null and _audio_swimming.playing:
			_audio_swimming.stop()
		return
